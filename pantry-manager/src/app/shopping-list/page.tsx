...
const mockStores = [
  { id: 'walmart', name: 'Walmart', estimatedTime: '2-3 hours', deliveryFee: 5.99 },
  { id: 'target', name: 'Target', estimatedTime: '3-4 hours', deliveryFee: 7.99 },
  { id: 'kroger', name: 'Kroger', estimatedTime: '1-2 hours', deliveryFee: 4.99 },
];
...
export default function ShoppingListPage() {
  ...
  const [newItemUnit, setNewItemUnit] = useState<'units' | 'g' | 'kg' | 'ml' | 'l'>('units');
  ...
  const generateSmartSuggestions = () => {
    ...
    pantryItems.forEach((item) => {
      const threshold = mockLowStockThresholds[item.name as keyof typeof mockLowStockThresholds];
    ...
    toast.success(`Added ${suggestions.length} smart suggestions to your shopping list!`);
  };
  ...
  const handleUpdateQuantity = (item: ShoppingListItem, newQuantity: number) => {
    if (newQuantity <= 0) {
  ...
  const totalCost = shoppingList.reduce((sum, item) => sum + (item.estimatedPrice || 0) * item.quantity, 0);
  const selectedStoreInfo = mockStores.find(store => store.id === selectedStore);
  ...
  // Priority items
  const highPriorityItems = shoppingList.filter(item => item.priority === 'high');
  const mediumPriorityItems = shoppingList.filter(item => item.priority === 'medium');
  const lowPriorityItems = shoppingList.filter(item => item.priority === 'low');
    ...
    <AppLayout pageTitle="Shopping List">
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-4">
          <Button onClick={generateSmartSuggestions} className="flex items-center gap-2">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select value={newItemUnit} onValueChange={(value) => setNewItemUnit(value as any)}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                {shoppingList.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">
                      Your shopping list is empty. Generate smart suggestions to get started!
                    </p>
                  </div>
                ) : (
          ...
          <TabsContent value="category" className="space-y-4">
            {Object.entries(groupedItems).map(([category, items]) => (
              <Card key={category}>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    {category === 'Low Stock' && <AlertTriangle className="h-5 w-5 text-orange-600" />}
                    {category === 'Recipe Ingredients' && <ChefHat className="h-5 w-5 text-blue-600" />}
                    {category === 'Expiring Soon' && <Clock className="h-5 w-5 text-red-600" />}
                    {category === 'Custom' && <Plus className="h-5 w-5 text-green-600" />}
                    {category}
          ...
          <TabsContent value="priority" className="space-y-4">
            {[
              { priority: 'high', items: highPriorityItems, color: 'text-red-600', label: 'High Priority' },
              { priority: 'medium', items: mediumPriorityItems, color: 'text-orange-600', label: 'Medium Priority' },
              { priority: 'low', items: lowPriorityItems, color: 'text-green-600', label: 'Low Priority' },
            ].map(({ priority, items, color, label }) => (
              items.length > 0 && (
                <Card key={priority}>
                  <CardHeader>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="flex justify-between font-semibold text-lg">
                  <span>Total</span>
                  <span>${(totalCost + (selectedStoreInfo?.deliveryFee || 0) + totalCost * 0.08).toFixed(2)}</span>
                </div>
              </div>
              <div className="mt-4 space-y-2">
                ...
                <p className="text-sm text-muted-foreground text-center">
                  Estimated delivery: {selectedStoreInfo?.estimatedTime} from {selectedStoreInfo?.name}
                </p>
              </div>
            </CardContent>
          </Card>
...
function ShoppingListItemCard({
  ...
  return (
    <div className={`flex items-center space-x-4 p-4 border rounded-lg border-l-4 ${
