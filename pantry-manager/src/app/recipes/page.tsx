...
export default function RecipesPage() {
  ...
  // Function to generate recipe suggestions wrapped in useCallback
  const generateRecipeSuggestions = useCallback(
    async (forceRefresh = false) => {
      // If force refresh is false and we have cached recipes, use them
      if (!forceRefresh) {
        const cachedRecipes = sessionStorage.getItem('pantryRecipes');
        if (cachedRecipes) {
      ...
      try {
        ...
        sessionStorage.setItem('pantryRecipes', JSON.stringify(data.recipes));
      } catch (error) {
    ...
    [pantryItems, dispatch, setSuggestedRecipes]
  );

  // Check for cached recipes on initial load
  useEffect(() => {
    if (pantryItems.length > 0) {
      generateRecipeSuggestions(false);
    }
  }, [pantryItems, generateRecipeSuggestions]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const assistantMessage: ChatMessage = {
        ...
        content:
          data.reply ||
          'Sorry, I cannot generate a response. Please try again.',
        timestamp: new Date(),
      };
  ...
  const categories = [
    { id: 'all', label: 'All Recipes', count: suggestedRecipes.length },
    {
      id: 'available',
      label: 'Can Make Now',
      count: suggestedRecipes.filter((r) => r.inPantry.length > 0 || 0).length,
    },
      ...
      <div className='space-y-6'>
        ...
        <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
          ...
          <div className='flex gap-2'>
            <Button
              onClick={() => generateRecipeSuggestions(true)}
              disabled={loadingRecipes}
              variant='outline'>
              <RefreshCw
                className={`h-4 w-4 mr-2 ${
                  loadingRecipes ? 'animate-spin' : ''
                }`}
              />
              Refresh Suggestions
            </Button>
            <Dialog>
              ...
              <DialogContent className='max-w-2xl max-h-[80vh]'>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Ask me anything about recipes, cooking tips, or meal
                    planning!
                  </DialogDescription>
                </DialogHeader>
        ...
        <Card>
          ...
          <CardContent>
            {pantryLoading ? (
              ...
              <p className='text-muted-foreground'>
                No pantry items found. Add some items to get personalized recipe
                suggestions!
              </p>
            )}
          </CardContent>
